...
function startMarquee(selector, speed, direction = 1) {
  ...
  const clones = line.querySelectorAll('.scroll-content.clone');
  clones.forEach(clone => clone.remove());

  // Duplicate content for seamless scroll
  const contentWidth = content.offsetWidth;
  const containerWidth = line.offsetWidth;
  let repeatCount = Math.ceil(containerWidth / contentWidth) + 2;
  ...
  function animate() {
    ...
    let offset = pos * -1;
    const allContents = line.querySelectorAll('.scroll-content');
    allContents.forEach((el, i) => {
      el.style.transform = `translateX(${offset + i * contentWidth}px)`;
    });
...
// Top line: left (-1), bottom line: right (1)
startMarquee('.scroll-line.scroll-left', 2, -1);
startMarquee('.scroll-line.scroll-right', 2, 1);
