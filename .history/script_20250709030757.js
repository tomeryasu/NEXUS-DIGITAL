...
// Desktop 'צור קשר' button scrolls to contact
const desktopContactBtn = document.getElementById('contact-btn');
...
function setActiveNavLink() {
  const sections = [
    'home', 'services', 'stat', 'about', 'clients', 'portfolio', 'case', 'results', 'contact'
  ];
  ...
  if (!found) {
    document.querySelectorAll('.nav-menu a, .mobile-menu-list a').forEach(link => link.classList.remove('active'));
  }
}
...
window.addEventListener('scroll', () => {
  if (window.scrollY > 80) {
    stickyContactBtn.style.display = 'block';
  } else {
...
// Header minimal mode on scroll (desktop only)
const mainHeader = document.querySelector('.main-header');
function handleHeaderMinimal() {
...
window.addEventListener('scroll', handleHeaderMinimal);
window.addEventListener('resize', handleHeaderMinimal);
document.addEventListener('DOMContentLoaded', handleHeaderMinimal);
...
function fillMarquee(lineSelector) {
  const line = document.querySelector(lineSelector);
  if (!line) return;
  const content = line.querySelector('.scroll-content');
  if (!content) return;

  // Remove any previous clones
  while (content.nextSibling) {
    content.parentNode.removeChild(content.nextSibling);
  }

  // Calculate how many times to repeat the content
  const containerWidth = line.offsetWidth;
  const contentWidth = content.scrollWidth;
  const repeatCount = Math.ceil(containerWidth / contentWidth) + 3; // +3 for extra seamlessness

  for (let i = 0; i < repeatCount; i++) {
    const clone = content.cloneNode(true);
    clone.style.position = 'absolute';
    clone.style.top = '0';
    clone.style.left = (i * contentWidth) + 'px';
    clone.style.height = '100%';
    content.parentNode.appendChild(clone);
  }
  // Set the original as absolute too
...
function startInfiniteScroll(selector, speed, direction = 1) {
  fillMarquee(selector);

  const line = document.querySelector(selector);
  if (!line) return;
  const contents = line.querySelectorAll('.scroll-content');
  ...
  function animate() {
    ...
    contents.forEach((el, i) => {
      el.style.transform = `translateX(${pos * -1 + i * contentWidth}px)`;
    });
...
// Left to right (direction = -1), speed increased to 2
startInfiniteScroll('.scroll-line.scroll-left', 2, -1);
// Right to left (direction = 1), speed increased to 2
startInfiniteScroll('.scroll-line.scroll-right', 2, 1);

// Refill on window resize and DOMContentLoaded for responsiveness and to fix empty spots
function refillAllMarquees() {
  fillMarquee('.scroll-line.scroll-left');
  fillMarquee('.scroll-line.scroll-right');
}
window.addEventListener('resize', refillAllMarquees);
document.addEventListener('DOMContentLoaded', refillAllMarquees);
